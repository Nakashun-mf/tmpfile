...
- **`make-open-url-exe.bat` / `run.bat`**  
  - URL を埋め込んだ exe を生成するメインのバッチファイルです。  
    （`run.bat` は Windows 上での文字コード対策用ラッパーで、中身は `bat_bukup.txt` を元に生成されています。）
...
- **1. ダブルクリックで起動する場合**
  - `make-open-url-exe.bat` をエクスプローラーからダブルクリックします。
...
- **2. コマンドラインから起動する場合**
  ...
  ```bat
  make-open-url-exe.bat URL [出力exe名]
  ```

  - 使用例:
    - `make-open-url-exe.bat https://example.com`
      - → `OpenUrl.exe` を生成
    - `make-open-url-exe.bat https://example.com MyShortcut.exe`
      - → `MyShortcut.exe` を生成
...
1. `make-open-url-exe.bat` をテキストエディタで開きます。
...
- `run.bat` はエディタや環境によっては文字化け対策のためエンコードを変換して生成しているため、**中身の編集は `bat_bukup.txt` 側で行うことを推奨**します。
